 ...
 */
function checkFilesWithTSOptions(sourceFilesPath: AbsPath[], tsConfOptions: ts.CompilerOptions) {
...
function parseTSConfig(tsconfPath: AbsPath) {
    ...
    if (fs.existsSync(tsconfPath)) {
        ...
        try {
            ...
            return reporters.throwError(EError.CANNOT_READ_TSCONFIG, `Cannot read provided project path '${tsconfPath}'. Make sure that project path is correct`);
        }
        ...
        if (tsconfJSON.error) {
            ...
            return reporters.throwError(EError.CANNOT_PARSE_TSCONFIG, `Cannot parse provided project '${tsconfPath}'. Make sure that tsconfig is valid config`);
        }
        ...
        return reporters.throwError(EError.NON_EXISTS_PROJECT_PATH, `Provided project path '${tsconfPath}' doesn't exists. Make sure that project path is correct`);
    }
}
...
function updateTSConf(tsconfPath: AbsPath, conf: any, sourceFilesPath: AbsPath[]) {
    const absDirName = path.dirname(tsconfPath);
    if (!existsSync(tsconfPath)) {
        reporters.throwError(EError.TSCONF_NOT_EXISTS, `Provided path '${tsconfPath}' for tsconf doesn't exists`);
    }
...
export function startApp(conf: AppConf): void {
    ...
    const shouldCheckJS = tsconfOptions.tsconfParsed.options.allowJs;

    console.log("Globing all files from src");
    const filesToCheck = globFiles(conf.srcPath, shouldCheckJS) as AbsPath[];
    if (filesToCheck.length === 0) {
        // TODO: Should throw an error ?
        reporters.throwError(EError.NO_FILES_TO_ANALYZE, `There are no typescript files in '${conf.srcPath}'`);
    }

    if (conf.verbose) {
        console.log(filesToCheck.join("\n"));
    }
    console.log(`Analyzing ${filesToCheck.length} files ...`);
